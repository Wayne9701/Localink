...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (
      ...
      output(
        await runtime.setExternalMcpRichImagePolicy(args[3], Number(args[4])),
      );
      return;
    }
    if (
      ...
      output(await runtime.setExternalMcpRichImagePolicy(args[3], undefined));
      return;
    }
    if (
      args.length === 4 &&
